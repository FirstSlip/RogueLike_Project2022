...
public class RoomGenerator : SimpleRandomWalkDungeonGenerator
{
    ...
    {
        roomsList = ProceduralGenerationAlgorithms.RoomSpawnerFirstLevel();
        ...
        WallsGenerator.CreateWalls(floor, tilemapVisualizer);
    }

    private HashSet<Vector2Int> ConnectRooms(List<Vector2Int> roomCenters)
    {
        HashSet<Vector2Int> corridors = new HashSet<Vector2Int>();
        graph = new List<Vector2Int>();
        var currentRoomCenter = roomCenters[0];
        roomCenters.Remove(currentRoomCenter);
        graph.Add(currentRoomCenter);
        while (roomCenters.Count > 0)
        {
            Vector2Int closest = roomCenters[0];
